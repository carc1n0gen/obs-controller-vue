...
export default function useSceneItems() {
  ...
  watch([isConnected, currentScene], async () => {
    if (isConnected.value && currentScene.value) {
      const res = await obs.value.call('GetSceneItemList', {
        sceneName: currentScene.value,
      })
      sceneItems.value = await res?.sceneItems.reduce(async (prevP, current) => {
        const prev = await prevP
        const activeRes = await obs.value.call('GetSceneItemEnabled', {
          sceneName: currentScene.value,
          sceneItemId: current.sceneItemId,
        })
        const sceneItem = {
          ...current,
          parentSceneName: currentScene.value,
          ...activeRes,
        }

        if (current.isGroup) {
          const groupRes = await obs.value.call('GetGroupSceneItemList', {
            sceneName: current.sourceName,
          })
          const groupSceneItems = groupRes.sceneItems.map((groupSceneItem) => {
            return {
              ...groupSceneItem,
              parentSceneName: sceneItem.sourceName,
            }
          })
